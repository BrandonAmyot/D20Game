...
int Dice::Roll(string rollOfDice){
	...
	lastRoll = 0;
	totalRolls ++;
	setState("Dice roll of: " + rollOfDice);

	//generates a random number depending on the type of the dice;
	for(int i = 0; i < numberOfDice; i ++){
		lastRoll = (rand() % kindOfDice) + 1;
		sumScore += lastRoll;
		cout << " Dice Roll " << (i+1) << ": = " << lastRoll << endl;
		
		std::string s = std::to_string(lastRoll);
		std::string p = std::to_string(i + 1);
		setState("Dice Roll " + p + " = " + s);
	}

	std::string q = std::to_string(sumScore);
	setState("Total Score Of Roll = " + q);
...
void Dice::setState(string newState) {
	_state = newState;
	Notify();
}
